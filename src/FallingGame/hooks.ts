...
// Images we want to preload regardless of remote config
...
export function useAssets() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const res = await fetch(`${CONFIG_URL}fallingGame?Id=1`, {
          cache: "no-store",
        });
        if (res.ok) {
      ...
      if (!effectiveConfig) return; // keep showing loader
...
export function useTries(storageKey: string = DEFAULT_KEY) {
  const [tries, setTriesState] = useState<number>(() =>
    parseCount(
      typeof window !== "undefined" ? localStorage.getItem(storageKey) : "0"
    )
  );
...
export function useGameSetup() {
  ...
  const { tries, noAttempts, decrementTries, setTries, resetTries } =
    useTries();
